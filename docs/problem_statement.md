# Project: Cerebro — Your Personal AI Second Brain
...
```mermaid
graph TD
    A[Capture any note/link/file] --> B[AI classifies & files it using PARA method]
    B --> C[AI auto-links it to related notes using embeddings]
    C --> D[Everything renders as a live, interactive, hoverable graph]
    D --> E[Ask it anything in plain English -> answer pulled from YOUR notes]
    E --> F[Deployed on a public URL anyone can open]
```
...
#### Deliverable ("Ship Cerebro" — the final product)
...
- [ ] All 4 weekly milestones complete (Capture Pipeline, Self-Organizing Wiki, Living Brain, Cerebro deployment)
...
### File Map
- `problemStatement.txt` or `problem_statement.md` → Store the problem that you are solving
...
1. Generate a detailed architecture for making this project using `@docs/problem_statement.md` (or `@PROBLEM_STATEMENT.md`).
2. Save the architecture into `architecture.md` file.
3. Generate a phase-wise `implementation-plan.md` using `@docs/architecture.md` and `@docs/problem_statement.md`.
